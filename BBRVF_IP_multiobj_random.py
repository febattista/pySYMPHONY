...
# server
sys.path.insert(0, r'/home/saf418/ValueFunctionCode/RVFCodes/SYMPHONYBBRVF/Test_py_IP_multiobj')
...
def calcRVFUBAtZeta(sampleZeta):
    ...
    for item in EF:
        added = True
        for i in range(1, numConsVar+1):
            if item[i] >= sampleZeta[i-1]:
                added = False
                break
        if added:
            filteredEF.append(item)
...
# with open("Results_IP_multiobj_details_{}.txt".format(instance.split('.')[0]), "a") as _filed:
